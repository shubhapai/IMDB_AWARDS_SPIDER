...
class TutorialPipeline(object):
	...
	def storeInDb(self, item):
		...
		for award in item['Awards']:
			self.storeAwardInfoInDb(award, filmID)			
	...
	def storeAwardInfoInDb(self, item, filmID):
		self.cur.execute("INSERT INTO Awards(\
			film_id, \
			award_title, \
			award_outcome, \
			award_category, \
		...
		(
			filmID,
			self.stripHTML(item.get('AwardTitle','')).strip(),
			self.stripHTML(item.get('AwardOutcome','')).strip(),
			self.stripHTML(item.get('awardCategory','')).strip(),
			self.stripHTML(item.get('awardDescription','')).strip(),
			self.stripHTML(item.get('awardOwner','')).strip()
		))
		self.con.commit()  
	...
	def createAwardsTable(self):
		self.cur.execute("CREATE TABLE IF NOT EXISTS Awards(id INTEGER PRIMARY KEY NOT NULL, \
			film_id INTEGER NOT NULL, \
			award_title TEXT, \
			award_outcome TEXT, \
			award_category TEXT, \
