...
from tutorial.items import MovieItem, CastItem, AwardItem
...
class tutorialSpider(scrapy.Spider):
	...
	start_urls = [
		"http://www.imdb.com/search/title?sort=moviemeter,asc&start=51&title_type=feature&year=2014,2014" # this is the top 250 list
	] 
	...
	def parseMovieDetails(self, response):
		...
		item = self.getAwardInfo(item, response)		
		return item
	...
	def getAwardInfo(self, item, response):
		for index,details in enumerate(response.xpath("//*[@id='maindetails_quicklinks']/div/ul/li")):
			titleDetails = details.xpath('a/text()').extract()
			print titleDetails
			if(titleDetails): 
				if ("Awards" in titleDetails):
					#print "in Awardslink"
					item['AwardsLink'] = "http://imdb.com"+details.xpath('a/@href').extract()[0] 
					request = scrapy.Request(item['AwardsLink'], callback=self.mapAwardDetails)
					request.meta['item'] = item
					yield request
		#return  item    

	def mapAwardDetails(self, response):
		item = response.meta['item']
		item['Awards'] = []
		for index, AwardInfo in enumerate(response.xpath("//*[@id='main']/div/div")):
			award = AwardItem()
			award['awardTitle'] = AwardInfo.xpath('h3/text()').extract() 
			award['awardOutcome'] = AwardInfo.xpath("//*[@class='title_award_outcome']/b/text()").extract()[0]
			award['awardCategory'] = AwardInfo.xpath("//*[@class='award_category']/text()").extract()[0]
			award['awardDescription'] = AwardInfo.xpath("//*[@class='award_description']/text()").extract()[0]
			award['awardOwner'] = AwardInfo.xpath("//*[@class='award_description']/a/text()").extract()[0]
